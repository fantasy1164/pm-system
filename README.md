...
> 以下畫面中的專案、契約號與金額皆為示範用的虛構資料；人名一律以去識別化格式（○）呈現。

### 進度總表

一個年度一頁。專案依狀態分組（進行中／未成案／已結案），欄位涵蓋契約號、SO number、
履約起迄、工期、參與人員、決標金額與**各年度預估認列**（跨年度專案分年列示）。
日期以民國年顯示。滑鼠移入表格會浮現「下載範本」按鈕——搭配批次匯入使用（見功能特色）。
...
- 批次匯入專案：下載 CSV 範本填寫後，拖曳到畫面任意位置即完成匯入（必填驗證、自動去重）
